...
(function (global) {
  ...
  function steps() {
    return [
      ...
      { id: 'typing-type', outlineGroup: 5, title: 'Copilot — typing', body: `Veil watches keystrokes too. Type or paste <em>any</em> secret in the compose box — e.g. <code>${TYPE_DEMO_KEY}</code>, the IBAN from before, or tap <strong>Copy key</strong> for a sample.`, type: 'action', target: '#practice-body', sampleCopy: TYPE_DEMO_KEY, sampleLabel: 'Copy key', waitFor: () => typingStepReady(), waitHint: 'Type or paste a secret — Veil flags it live…', waitReadyHint: 'Veil caught it — tap Continue when ready.', manualAdvance: true, showContinueDuringWait: true, continueLabel: 'Continue', prime: async () => { await prepareCopilotLab(); resetCopilotForTyping(); focusComposeEnd(); } },
